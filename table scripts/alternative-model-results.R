...
for (i in 1:4) {
  ...
  adj_reduction <- reduction/vacc_coverage[i]*100
  adj_reduction[adj_reduction>100] <- 100
  ...
  tbl <- rbind(tbl, list(format_res(cases), format_res(averted), format_res(adj_reduction), format_res(reduction)))
}

reduction <- total_averted/total_cases*100 
adj_reduction <- round(reduction/vacc_avg[5]*100)
cases <- format(round(total_cases, -2), big.mark=",")
averted <- format(round(total_averted, -2), big.mark=",")
reduction <- round(reduction)

tbl[1,] <- list(format_res(cases), format_res(averted), format_res(adj_reduction), format_res(reduction))
names(tbl) <- c("Predicted COVID-19 cases (95% UI)", "Averted cases COVID-19 (95% UI)", 
                "Adjusted relative reduction in % (95% UI)", "Relative reduction in % (95% UI)")
tbl[["Age group (years)"]] <- c("Total 12+", "12-17", "18-49", "50-64", "65+")
tbl <- select(tbl, 5, 1, 2, 3, 4)
