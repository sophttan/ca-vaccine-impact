###################################################################################################
#Title: Figures for sensitivity analysis of alternative analysis - accounting for
# reduced vaccine effectiveness against delta variant
#Author: Sophia Tan
###################################################################################################
...
rm(list=ls())
setwd(here::here())
source("figures/alternative/scripts/alternative-model-plot-functions.R")
...
(p1 + theme(axis.title.x = element_blank(), axis.title.y.right = element_blank()) + labs(title="A", subtitle="12-17 years") +
  ...
  ggsave(file="Direct Effects Analysis/final plots/alternative-model-delta-mc-case-predictions.png", dpi=300, width=7, height=7.5)
...
(plot_relative_reduction(res_12_18) + labs(title="A", subtitle="12-17 years") + theme(axis.title.x = element_blank()) +
  ...
  ggsave(file="Direct Effects Analysis/final plots/alternative-model-delta-relative-reduction.png", dpi=300, width=7, height=7.5)
