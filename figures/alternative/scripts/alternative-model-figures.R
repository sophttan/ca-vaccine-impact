...
(p1 + theme(axis.title.x = element_blank(), axis.title.y.right = element_blank()) + labs(title="A", subtitle="12-17 years") +
  ...
  ggsave(file="Direct Effects Analysis/final plots/alternative-model-mc-simulation-cases.png", dpi=300, width=7, height=7.5)
...
(plot_relative_reduction(res_12_18) + labs(title="A", subtitle="12-17 years") + theme(axis.title.x = element_blank()) +
  ...
  ggsave(file="Direct Effects Analysis/final plots/alternative-model-mc-simulation-relative-reduction.png", dpi=300, width=7, height=7.5)
