###################################################################################################
#Title: Functions for estimating averted hospitalizations and deaths in CA using susceptibility-based model
#Author: Sophia Tan
###################################################################################################

# make table that has vaccine effectiveness over time by manufacturer
# option to make table with effectiveness against the delta variant
make_vacc_table <- function(row, delta) {
  ...
  if (delta==T) {
    ...
    d$jj <- ifelse(d$weeks < avg_time_jj, row$jj, row$jj_wane)
    d$p1 <- row$p1
...
# take case/hosp/death data and susceptibility data to calculate incidence over time in each vaccine-eligible age group
prep_data <- function(data, sus) {

  d <- data %>% merge(sus, by="weeks_since_Jan2020")
...
# make hosp/death predictions from estimated incidence - estimate total immune from natural immunity only
make_case_predictions <- function(data, age, inf_ages, asymp_prop, susceptible, rate, case) {
  ...
  for (i in 1:nrow(data)) {
    ...
    if(row[[rate]]==0){
      ...
      cases <- cases_no_vacc/(row[[rate]]/100)
    }

    inf_novacc <- 0

    for (j in 1:length(inf_ages)) {
      inf_novacc <- inf_novacc + cases*inf_prop[j]/(1-asymp_prop[j])
    }
