...
intended.quantiles <- c(0.64, 0.93)
intended.mean <- 0.84
opt_jj <- optimal()
parameters$jj <- qbeta(p[,1], opt_jj[1], opt_jj[2])

intended.quantiles <- c(0.49, 0.80)
intended.mean <- 0.68
opt_jj <- optimal()
parameters$jj_wane <- qbeta(p[,1], opt_jj[1], opt_jj[2])

# delta
intended.quantiles <- c(0.31, 0.77)
intended.mean <- 0.60
opt_jj <- optimal()
parameters$jj_delta <- qbeta(p[,1], opt_jj[1], opt_jj[2])


intended.quantiles <- c(.21, .92)
intended.mean <- 0.75
opt_p1 <- optimal()
parameters$p1 <- qbeta(p[,2], opt_p1[1], opt_p1[2])

intended.quantiles <- c(0.9, 0.98)
intended.mean <- 0.95
opt_p2 <- optimal()
...
# delta
intended.quantiles <- c(.49, .97)
intended.mean <- 0.79
opt_p1 <- optimal()
parameters$p1_delta <- qbeta(p[,2], opt_p1[1], opt_p1[2])

intended.quantiles <- c(0.84, 0.96)
intended.mean <- 0.93
opt_p2 <- optimal()
...
saveRDS(parameters, "data/simulated-parameters-hosp.RDS")
