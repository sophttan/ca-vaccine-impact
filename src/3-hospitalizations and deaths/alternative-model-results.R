...
hosp_counts <- readRDS("Direct Effects Analysis/Hospitalizations and deaths/hosp_rate_week_month_rates.RDS") %>% 
  select(weeks_since_Jan2020, age_hand_cut, num_hosp) %>% spread(age_hand_cut, num_hosp, fill=0) 
vacc_spread <- readRDS("Direct Effects Analysis/Data/vaccination_coverage_data.RDS")
res_18_50 <- readRDS("Direct Effects Analysis/Hospitalizations and deaths/results/res_18_50_hosp.RDS")%>% filter(weeks_since_Jan2020 >= 48)
res_50_65 <- readRDS("Direct Effects Analysis/Hospitalizations and deaths/results/res_50_65_hosp.RDS")%>% filter(weeks_since_Jan2020 >= 48)
res_65 <- readRDS("Direct Effects Analysis/Hospitalizations and deaths/results/res_65_hosp.RDS")%>% filter(weeks_since_Jan2020 >= 48)
...
tbl %>% write_csv("Direct Effects Analysis/Hospitalizations and deaths/alternative-model-sim-hosp-tbl.csv")

p1 <- plot_hosp_predictions(res_18_50) + 
  scale_y_continuous(labels = comma, limits = c(0, max(res_65$ub)) ) + labs(title="18-49 years")

p2 <- plot_hosp_predictions(res_50_65) + ylim(0, max(res_65$ub)) + 
  scale_y_continuous(labels = comma, name = NULL, limits = c(0, max(res_65$ub)) ) + labs(title="50-64 years")

p3 <- plot_hosp_predictions(res_65) + ylim(0, max(res_65$ub)) + 
...
(p1 + p2 + p3 + plot_layout(guides = "collect", nrow=1) & theme(legend.position = "bottom")) %>%
  ggsave(file="Direct Effects Analysis/Hospitalizations and deaths/alternative-model-mc-simulation-hosp.png", dpi=300, width=9.5, height=4)
