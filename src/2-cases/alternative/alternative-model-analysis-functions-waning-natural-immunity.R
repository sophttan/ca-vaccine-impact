...
# accounts for waning natural immunity and interaction between prior natural infection and subsequent vaccination
#Author: Sophia Tan
###################################################################################################
...
calculate_vacc <- function(group, d, delta=F) {
  ...
  if (group==1) {
    data$total_inf <- c(0, data$total_inf_12_18[1:nrow(data)-1])
  ...
  for (j in (vaccination_begins+1):nrow(data)){
    ...
    prop_inf <- data$total_inf_adj[1:j]/sum(data$total_inf_adj[1:j])
    will_be_vacc <- (prop_inf)*(row$vacc-new)
    for (g in (1:j)){
      inf <- will_be_vacc[g]
      if (j-g-1 > 52) {
        data$immune_prev_inf <- data$immune_prev_inf + inf*c(rep(0, g-1), rep(params$nat_wane, 52), rep(params$nat_wane1, j-52-g), rep(.9, nrow(data)-j+1))
      } else{
        if(nat_bigger&g<42){
          data$immune_prev_inf <- data$immune_prev_inf + inf*c(rep(0, g-1), rep(params$nat_wane, 52), rep(.9, nrow(data)-52-g+1))
        }else if(nat_bigger){
          data$immune_prev_inf <- data$immune_prev_inf + inf*c(rep(0, g-1), rep(params$nat_wane, nrow(data)-g+1))
        } else{
          data$immune_prev_inf <- data$immune_prev_inf + inf*c(rep(0, g-1), rep(params$nat_wane, j-g), rep(.9, nrow(data)-j+1))
        }
      }
    }

    data$total_inf_adj[1:j] <- data$total_inf_adj[1:j]-will_be_vacc
    new_vaccs <- c(new_vaccs, new)
  }

  for (j in 1:nrow(data)) {

    num_inf <- data$total_inf_adj[j]
    if (j < 42) {
      data$immune_prev_inf <- data$immune_prev_inf + num_inf*c(rep(0, j-1), rep(params$nat_wane, 52), rep(params$nat_wane1, nrow(data)-j-52+1))
    } else {
      data$immune_prev_inf <- data$immune_prev_inf + num_inf*c(rep(0, j-1), rep(params$nat_wane, nrow(data)-j+1))
    }
  }
  ...
  susceptible <- pop_age[group] - (data$full_immune_vacc + data$immune_prev_inf)
...
make_case_predictions <- function(data, age, inf_ages, asymp_prop, susceptible, model_type) {
  inf <- rep(0, nrow(data))
  cases_novacc_update <- rep(0, nrow(data))
  ...
  for (i in 1:nrow(data)) {
    ...
    cases_no_vacc <- pred*(total_sus-inf[i])
    ...
    if (i < nrow(data)) {
      ...
      if (i < 42) {
        inf[(i+1):nrow(data)] <- inf[(i+1):nrow(data)] + c(rep(inf_novacc*params$nat_wane, 52), rep(inf_novacc*params$nat_wane1, nrow(data)-i-52))
      } else {
        inf[(i+1):nrow(data)] <- inf[(i+1):nrow(data)] + rep(inf_novacc*params$nat_wane, nrow(data)-i)
      }
    }

    cases_novacc_update[i] <- cases_no_vacc
