...
starting.params<-c(20, 2)
set.seed(13)
p <- runif(n, 0, 1)
intended.quantiles <- c(0.27, 0.97)
intended.mean <- 0.86
opt_nat_wane <- optimal()
parameters$nat_wane <- qbeta(p, opt_nat_wane[1], opt_nat_wane[2])

intended.quantiles <- c(0.38, 0.84)
intended.mean <- 0.69
opt_nat_wane <- optimal()
parameters$nat_wane1 <- qbeta(p, opt_nat_wane[1], opt_nat_wane[2])
